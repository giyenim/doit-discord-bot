...
const MESSAGES = {
  ...
  publicPending: (user, label) => `${user} 님의 [${label}] 인증 요청이 접수되었어요!\n관리자 확인 후 승인해 드릴게요. 평일 기준 1일 이내에 처리되니 조금만 기다려 주세요:)`,
  adminRequest: (user, label) => `${user} (${user.tag}) 님이 [${label}] 인증을 요청했습니다.`,
...
const commands = [
  new SlashCommandBuilder()
    .setName('인증')
    .setDescription('인증을 요청합니다 (역할 선택 + 이미지 첨부 필요)')
    .addStringOption((option) =>
      option
        .setName('스터디')
        .setDescription('인증할 역할을 선택하세요')
        .setRequired(true)
        .addChoices(
          { name: process.env.ROLE_LABEL_OPTION_1, value: 'opt1' },
          { name: process.env.ROLE_LABEL_OPTION_2, value: 'opt2' },
        )
    )
...
async function runVerifyCommand(interaction) {
  ...
  const optKey = interaction.options.getString('스터디');
  const { label } = OPTIONS[optKey];
  ...
  const publicMessage = await interaction.editReply({
    content: MESSAGES.publicPending(interaction.user, label),
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`approve_${optKey}_${suffix}`)
      .setLabel('승인')
  ...
  await adminChannel.send({
    content: MESSAGES.adminRequest(interaction.user, label),
    files: [attachment.url],
    components: [row],
  });
}
...
async function handleButton(interaction) {
  ...
  let optKey, userId, messageId, channelId;
  if (action === 'approve') {
    [, optKey, userId, messageId, channelId] = parts;
  } else {
    [, userId, messageId, channelId] = parts;
  }
  ...
  const { roleId, label } = OPTIONS[optKey];
